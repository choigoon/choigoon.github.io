...
var UI = {
  ...
  lnb : function(){
    ...
    $lnb.on("mouseleave",function(e){
      $lnbMenu.removeClass('is-active');
    })
  },

  gnb : function(){
    $(".btn_util_gnb").on("click keydown",function(e){
      if (e.type === 'keydown') {
      ...
      if($(this).hasClass("is-open")){
        // gnbAccr
        var $allAccr = $(".gnb .gnb_dep2_tit");

        $allAccr.each(function(i,el){
          var $dep3 = $(el).siblings(".gnb_dep3");
          var dep3HgtPx = $dep3.outerHeight(true);

          // Accr 초기화
          $dep3.css("height", 0);

          $(el).on("click focus",function(){
            var $elLi = $(el).parent(".gnb_dep2_li");
            var isActive = $elLi.hasClass("is-active");

            // 모든 Accr 초기화
            $allAccr.parent(".gnb_dep2_li").removeClass("is-active");
            $allAccr.find(".blind").text("목록 열기");
            $allAccr.siblings(".gnb_dep3").css("height", 0);

            if(isActive){
              $elLi.removeClass("is-active");
              $(el).find(".blind").text("목록 열기");
            }else {
              $elLi.addClass("is-active").find(".gnb_dep2_tit .blind").text("목록 닫기");
              $dep3.css("height", dep3HgtPx);
            }
          });
        });
      }
